...
async function createProductionUser() {
  try {
    ...
    const existingAdmin = await User.findOne({ email: 'admin@quizknow.com' });
    if (existingAdmin) {
    ...
    const existingInstructor = await User.findOne({ email: 'instructor@quizknow.com' });
    ...
    const existingStudent = await User.findOne({ email: 'student@quizknow.com' });
    if (existingStudent) {
