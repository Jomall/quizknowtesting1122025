...
// Configure multer for file uploads (only in non-Vercel environment)
let upload;
if (!process.env.VERCEL) {
  ...
  // For Vercel, use memory storage or skip multer setup
  upload = multer({
    storage: multer.memoryStorage(),
    limits: {
      fileSize: 5 * 1024 * 1024 // 5MB limit for Vercel (maximum allowed)
    }
  });
}
